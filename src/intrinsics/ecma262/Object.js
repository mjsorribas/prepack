...
export default function(realm: Realm): NativeFunctionValue {
  ...
  let ObjectAssign = func.defineNativeMethod("assign", 2, (context, [target, ...sources]) => {
    ...
    for (let nextSource of sources) {
      ...
      if (HasSomeCompatibleType(nextSource, NullValue, UndefinedValue)) {
        ...
        if (to_must_be_partial) {
          if (to instanceof AbstractObjectValue && to.values.isTop() && to.isSimpleObject()) {
            // We know to is well behaved, but we don't know any of its properties.
            // Hence the fact that we also know nothing about the properties of frm does not cause a problem for us.
            // Consequently, generate a residual Object.assign call that copies the properties that we don't know about.
            ...
            // if to has properties, we better remove them because after this we don't know their values anymore
            if (to.hasProperties()) {
              temporalTo = AbstractValue.createTemporalFromBuildFunction(
                realm,
                ObjectValue,
                [ObjectAssign, to, to.getSnapshot()],
                ([methodNode, targetNode, sourceNode]: Array<BabelNodeExpression>) => {
                  return t.callExpression(methodNode, [targetNode, sourceNode]);
                }
              );
            ...
            if (frm_was_partial) {
              temporalTo = AbstractValue.createTemporalFromBuildFunction(
                realm,
                ObjectValue,
                [ObjectAssign, temporalTo || to, frm.getSnapshot()],
                ([methodNode, targetNode, sourceNode]: Array<BabelNodeExpression>) => {
                  return t.callExpression(methodNode, [targetNode, sourceNode]);
                }
              );
    ...
    // 5. Return to.
    if (to_must_be_partial) {
      // generate Object.assign for remaining properties
      if (to.hasProperties()) {
        temporalTo = AbstractValue.createTemporalFromBuildFunction(
          realm,
          ObjectValue,
          [ObjectAssign, temporalTo || to, to.getSnapshot()],
          ([methodNode, targetNode, sourceNode]: Array<BabelNodeExpression>) => {
            return t.callExpression(methodNode, [targetNode, sourceNode]);
          }
        );
      ...
      // Tell serializer that it needs to add properties to to only after temporalTo has been emitted
      if (temporalTo instanceof AbstractObjectValue) to.addtemporalAlias(temporalTo);
    }
    return to;
  });
